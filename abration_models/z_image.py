...
class SD3Model(nn.Module):
    ...
    def train_model(self, x_0, train_prompt: str):
        ...
        for _ in progress_bar:
            ...
            t_scaler = torch.rand(1).to(self.device) # 0.0-1.0の一様乱数
    ...
    def forward(
        ...
        control_image = canny_process(input_image)
        control_latent = pil_to_latent(self.pipeline, control_image, self.device, self.dtype)
        self.control_latent = control_latent.unsqueeze(2)
        ...
        control_image_in = self.control_latent.repeat(2, 1, 1, 1, 1)
...
if __name__ == "__main__":
    ...
    for idx, frame in enumerate(frames):
        frame.save(f"temp_saves/zimage_{date}_{idx}.png")
